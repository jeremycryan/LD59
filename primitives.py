...
class GameObject:
    def __init__(self, game):
        self.game = game
        self.radius = 100
        self.base_point = Pose((0, 0))
        self.position = Pose((0, 0))
        self.is_destroyed = False

    def get_base_position(self):
        return self.position + self.base_point
    ...
    def draw_shadow(self, surf, offset=(0, 0), scale=1):
        ...
        y = self.position.y*scale + self.base_point.y*scale + offset[1]*scale - shadow.get_height()//2

        surf.blit(shadow, (x, y), special_flags=pygame.BLEND_MULT)

    @staticmethod
    def nudge(obj_with_radius, obj_with_size):
        if not obj_with_size.collides_with(obj_with_radius):
            return
...
def scale_surface_by(surf, factor):
    return pygame.transform.scale(surf, ((surf.get_width()*factor)+1, surf.get_height()*factor+1))